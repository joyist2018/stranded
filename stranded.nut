...
function EasyLogic::Triggers::inv ( player, args, text )
{
	...
	Utils.SayToAllDel("Hold SHIFT to select an option or RIGHT-CLICK to switch options.");
	Utils.SayToAllDel("%s is viewing his/her inventory.", player.GetName());
	Utils.SetEntityHint( player, "SHIFT key to select option, RIGHT-CLICK to switch options", "icon_info", 0, false, 20.0, 0 );
}
